...
**Recently fixed (verified by the test suite — kept here so stale references below don't mislead)**
- **DELETE now deletes** (`std::remove` at the end of `handle_setup`, guarded on `method == DELETE && internal && !no_file && status == 200`; responds 204, `no_file = true`). ⚠️ **Test-authoring rule learned the hard way:** DELETE tests must create their own scratch file — a stale `method_test` doing `DELETE /index.html` wiped `www/index.html` on the first run after DELETE worked, and every later suite failed with 302s (the file is untracked in git, so it had to be reconstructed).
- **Binary files serve intact.** `ScratchBuffer::fill(fstream)` now uses `read()`/`gcount()` (count-based) instead of `get(..., '\0')`. Note `find()`/`strnstr` are still NUL-stopping — irrelevant for serving, only touches CGI header scanning.
- **CGI env now sets `CONTENT_LENGTH` and `CONTENT_TYPE`** (POST-with-body / header-present conditionals per RFC 3875).
- **`handle_cgi_output` rewritten** (deferred whole-block header parse): LF *or* CRLF line endings; the blank line is consumed (stray-leading-CRLF bug gone); `Status:` matched case-insensitively; missing `Status:` defaults to a proper `200` status line emitted *first*; a CGI that dies before completing its headers (or overflows the buffer) gets a clean `502` instead of a 100 % CPU spin; finalize happens only at true EOF (`read() == 0`), not on the HUP flag, so large CGI bodies are no longer truncated. A transcription bug during this rewrite (missing `line_start` offset — every header line after the first parsed as a prefix of line 1) is pinned by `test-cgi-multi-header-response-intact` + `www/cgi-bin/multi_header.py`.
- **`/tmp/cgi_*` temp files are unlinked** in `finalize_cgi` (unlink-while-open idiom: name removed immediately, inode lives until the stream closes).
- **`EPOLLHUP | EPOLLHUP` duplicate-flag hangup check** fixed (commit `bee2077`), then superseded by the EOF-driven finalize above.
...
**Correctness bugs**
- **Parse-error status is clobbered — and now yields 2xx/3xx, not just 405.** `handle_req_line`/`handle_req_headers` set e.g. 400/501/505, but `handle_setup()` unconditionally resets `_req.status` to 200/201 and routes normally. Verified at runtime: `GET / HTTP/2.0` and garbage-version requests return **200** (they returned 505/400 in earlier runs — the error-response tests for these regressed during recent `handle_setup` work); unknown methods (OPTIONS/PUT/HEAD/PATCH) return 405 instead of 501; a header line with no colon is *detected* (the `if (!colon)` check was fixed to also test `npos`) but the 400 is clobbered into a 200. One fix — don't reset a non-2xx `_req.status`, route to `epi_redirect` instead — would un-fail the whole `error-response`/`parse-error` family. Subject: "your HTTP response status codes must be accurate."
- **Every 404 becomes `302 → google.com` under `base.conf`.** `conf/base.conf` has two `error_page 404` lines; `config::errors::add_page` is `_pagemap[code] = epi` so the **last** duplicate wins (nginx keeps the first). Breaks `test-missing-file-is-error` and `test-missing-cgi-script-is-error`; also disguises the parse-error clobber (400→404→302). Fix: drop the experiment line from base.conf and/or make duplicates first-wins.
- **413 is unreachable on non-CGI routes.** In `handle_setup` the method-allow check runs before the body-size check, and `is_method_allowed()` rejects POST on any location without `cgi_pass` — so an oversized POST to a static route gets 405, never 413 (`test-oversized-body-rejected`). `client_max_body_size` only functions on CGI locations.
- **Header *values* are lowercased wholesale.** `handle_req_headers` lowercases the entire line before splitting (`ClientConnection.cpp:400`), corrupting case-sensitive values — multipart boundaries, cookies. Blocks CGI-based multipart upload; fix by lowercasing only the key.
...
**Newly found (verified at runtime)**
...
- ~~CGI header parsing is CRLF-only~~ / ~~CGI without `Status:` has no status line~~ — **fixed** by the `handle_cgi_output` rewrite (see the Fixed block above).
...
**Additional issues found by source review (second pass)**
...
- **`finalize_cgi()` blocks the event loop in `waitpid(pid, NULL, 0)`** — a CGI that closes stdout but keeps running (sleeps, daemonizes) stalls the entire single-threaded loop until the child exits; every other client hangs. The same blocking `waitpid` was copied into the new 502 branch of `handle_cgi_output`. Needs `WNOHANG` + reap-later, or `kill` then reap (stdout is already at EOF when finalizing, so killing is safe). Related missing piece: no CGI runtime timeout at all — an immortal script holds its connection forever. _(Regression: cgi-robustness slow-exit responsiveness probe — still failing.)_
- ~~CGI output truncated on `EPOLLHUP`~~ / ~~CGI exiting before a complete header line pegs a core~~ — **fixed** by the `handle_cgi_output` rewrite (EOF-driven finalize; incomplete-headers-at-EOF → 502).
...
- ~~CGI body gets a stray leading CRLF~~ — **fixed**; the rewritten header parse consumes the blank line.
...
- **A CGI header block larger than the buffer is answered with 502, not forwarded.** The rewritten parser waits for the complete header block in the scratch buffer; if it can never fit, the connection now gets a clean 502 (previously it wedged mid-response via `buffer_res_headers`). Forwarding oversized headers would need growable header buffering — policy decision pending. _(cgi-robustness big-header probe still fails: it expects the header forwarded.)_
- **Weak value validation.** `Content-Length` parsing (`ClientConnection.cpp:452-455`) accepts trailing junk (`123abc`), negatives wrap through `size_t`, and a fully unparseable value returns **500** (a client error reported as a server error); the version check `find_first_not_of('0', 8)` accepts `HTTP/1.10`/`HTTP/1.100`. (CGI `Status` case-sensitivity is fixed — `equals_icase`.) _(Regressions: parse-error Content-Length-junk and version-1.10 tests.)_
...
- **`mkstemp`** (in `handle_cgi_output`) is not on the subject's allowed-functions list — an evaluation flag, same category as fork-for-autoindex; replaceable with `open(O_CREAT|O_EXCL)` on a generated name. `bzero`/`strncpy` are POSIX-isms; prefer `memset` / C++ forms given the "prefer C++ versions" rule.
- **Untracked test assets break fresh clones.** `conf/redirect_buffer.conf` *and the `www/` assets* (`index.html` was reconstructed by hand after the DELETE-test incident) are not in git, yet the suite depends on them. `git add` the lot, or `make test` on a fresh clone fails in confusing ways.
- Cosmetic: `"HTTP Version Not SUpported"` typo (`Response.cpp:39`); `parse_cgi_headers()` is now dead code (superseded by the in-place deferred parse).

Priority order for fixes (current suite standing: 89 tests / 34 failures, down from 43): the half-close busy-loop and the `%25` decode loop first — both are remotely triggerable, both violate "must remain operational / never hang indefinitely", and either alone is an evaluation-failing DoS. Then the `ScratchBuffer::feed` `size_t` wrap (crash on a client that closes mid-response), the parse-error status clobber (accurate status codes are a subject requirement, and bad-version requests currently return 200), timeout enforcement (which also bounds the blocking-`waitpid`/immortal-CGI hangs), then file upload, then the config-feature bugs (internal `return`, `index` append, header buffer size, Content-Type).
